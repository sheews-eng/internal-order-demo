...
function initSalesmanPage() {
  ...
  const render = () => {
    ...
    Object.entries(grouped).forEach(([customer, customerOrders]) => {
      ...
      const itemsStr = customerOrders.map(o => `${o.item} x${o.quantity} ($${o.price || 0})`).join(', ');
...
function initAdminPage() {
  ...
  const render = () => {
    ...
    Object.entries(grouped).forEach(([key, orderArr])=>{
      const [salesman, customer] = key.split('|');
      const itemsStr = orderArr.map(o=>`${o.item} x${o.quantity} ($${o.price||0})`).join(', ');
      const status = orderArr.every(o=>o.status==='Completed')?'Completed':
                     orderArr.every(o=>o.status==='Ordered')?'Ordered':'Pending';
      const tr = document.createElement('tr');
      tr.innerHTML = `
        <td>${salesman}</td>
        <td>${customer}</td>
        ...
        <td>${status}</td>
        <td>
          ${status!=='Completed'?`<button onclick="markCompletedCustomer('${salesman}','${customer}')">Mark Completed</button>`:''}
        </td>
      `;
      tableBody.appendChild(tr);
    });
  ...
  window.markCompletedCustomer = (salesman, customer) => {
